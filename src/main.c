...
void Euler_integrate() {
    ...
    for (int i = 0; i < 4; i++) {
        q[i] += dt * qdot[i];
        qdot[i] += dt * qddot_temp[i];
    }
}

void symplectic_integrate() {
    float qddot_temp[4];
    compute_qddot(q, qdot, qddot_temp);
    for (int i = 0; i < 4; i++) {
        qdot[i] += dt * qddot_temp[i];
...
void verlet_integrate() {
    ...
    for (int i = 0; i < 4; i++) {
        float temp = q[i];
        q[i] = 2.0f * q[i] - q_prev[i] + dt * dt * qddot_temp[i];
        qdot[i] = (q[i] - q_prev[i]) / (2.0f * dt);
        q_prev[i] = temp;
    }
}
...
int main(){
	...
	compute_qddot(q, qdot, qddot_temp);
	for (int i = 0; i < 4; i++) {
		q_prev[i] = q[i] - dt * qdot[i] + 0.5f * dt * dt * qddot_temp[i];
	}
	IntegrationMethod method = VERLET_METHOD;
	const char* output_filename = "energy_ver.csv";
